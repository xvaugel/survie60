...
import { getEnemyProfile, getEnemyTypeForLevel, spawnEnemyOfType, spawnBoss, moveEnemies as moveEnemiesFromFactory } from './EnemyFactory.js';
...
export class SurvivalScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.coinsText = this.add.text(20, 125, 'Pièces : 0', { fontSize: '20px', color: '#facc15' });
    this.infoText = this.add.text(20, 152, 'Déplacement : ZQSD / WASD / flèches', { fontSize: '18px', color: '#cccccc' });
    this.weaponText = this.add.text(20, 177, 'Arme : canon auto souris', { fontSize: '18px', color: '#cccccc' });
    this.shopBonusText = this.add.text(20, 202, `Boutique : VIT ${this.runUpgrades.speed} | TIR ${this.runUpgrades.fireRate} | BAL ${this.runUpgrades.bulletSpeed}`, { fontSize: '16px', color: '#94a3b8' });
    this.runStatsText = this.add.text(20, 224, `Run : Pierce ${this.bulletPierce}`, { fontSize: '16px', color: '#93c5fd' });
    createLevelUI(this);
  }
  ...
  update(time, delta) {
    ...
    if (updateLevelTimer(this, delta)) {
      return;
    }
  ...
  getEnemyProfile(type) {
    return getEnemyProfile(type);
  }

  getEnemyTypeForCurrentLevel() {
    return getEnemyTypeForLevel(this.currentLevel);
  }
  ...
  spawnEnemyOfType(type, x, y, time) {
    return spawnEnemyOfType(this, type, x, y, time);
  }
  ...
  showMultiKillText(x, y, count) {
    const txt = this.add.text(x, y - 26, `${count} HIT`, {
      fontSize: '18px',
      color: '#60a5fa',
      fontStyle: 'bold',
    }).setOrigin(0.5);
  ...
  updateBossUI() {
  // temp: évite le crash tant que l'UI boss n'est pas recâblée
  }
